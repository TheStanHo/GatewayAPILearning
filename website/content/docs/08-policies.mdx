...
- **Nginx Gateway Fabric**: Policy support is evolving. Check the [NGF documentation](https://docs.nginx.com/nginx-gateway-fabric/) for current policy support status. Note that NGF does not support extended fields in HTTPRoute rules such as `timeouts`, `retries`, or `sessionPersistence` - these fields will be ignored if defined in HTTPRoute rules.
...
👉 **[Next: Migration Guide →](/docs/09-migration-guide)**
